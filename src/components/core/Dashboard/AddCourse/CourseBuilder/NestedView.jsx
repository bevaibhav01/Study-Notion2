...
export default function NestedView({ handleChangeEditSectionName }) {
  ...
  // States to keep track of modal visibility
  const [addSubSection, setAddSubsection] = useState(null)
  const [viewSubSection, setViewSubSection] = useState(null)
  const [editSubSection, setEditSubSection] = useState(null)
  const [addQuiz, setAddQuiz] = useState(null) // State for Add Quiz modal
  const [confirmationModal, setConfirmationModal] = useState(null)
    ...
    <>
        ...
        {course?.courseContent?.map((section) => (
          <details key={section._id} open>
            ...
            <div className="px-6 pb-4">
              ...
              {/* Add Quiz to Section */}
              <button
                onClick={() => setAddQuiz(section._id)} // Set the section ID for the quiz
                className="mt-3 flex items-center gap-x-1 text-yellow-50"
              >
                <FaPlus className="text-lg" />
                <p>Add Quiz</p>
      ...
      {addSubSection ? (
        ...
        <SubSectionModal
          ...
          edit={true}
        />
      ) : addQuiz ? ( // Quiz modal condition
        <QuizModal
          sectionId={addQuiz} // Pass the section ID to the QuizModal
          setModalData={setAddQuiz}
          add={true} // Indicate that it's an add operation
        />
      ) : (
        <></>
      )}
